...
Fonctions :
  ...
  parse_dependabot_alert()   — Parse un payload GitHub Dependabot Alert
...
def parse_dependabot_alert(payload: dict) -> Optional[dict]:
    """
    Parse un payload GitHub dependabot_alert webhook.

    Format : https://docs.github.com/en/webhooks/webhook-events-and-payloads#dependabot_alert
    — contrairement à security_advisory (une GHSA globale, indépendante d'un
    repository précis), un Dependabot alert est toujours scopé à un paquet et
    un fichier manifeste précis d'un repository donné (alert.dependency).

    Extrait l'ID CVE depuis alert.security_advisory.cve_id.

    Returns:
        dict avec {cve_id, severity, description, ghsa_id, package, ecosystem,
        manifest_path, alert_state, action, source} ou None si aucune CVE
        n'est identifiable — une alerte peut référencer une GHSA sans CVE
        assignée pour l'instant, auquel cas il n'y a rien à corréler avec les
        manifests internes (indexés par ID CVE, pas par GHSA).
    """
    ...
    return {
        ...
        "action":        payload.get("action"),
        "source":        "github_dependabot_alert",
    }
