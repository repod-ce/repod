...
os.environ.setdefault("STAGING_INCOMING",       os.path.join(_TMP, "staging_incoming"))
os.environ.setdefault("STAGING_QUARANTINE",     os.path.join(_TMP, "staging_quarantine"))
...
class TestSettingsRouterGPGAndMore:
    ...
    def test_generate_gpg_key_triggers_distributions_reinit(self, sr_client):
        """
        Une clé nouvellement générée doit être immédiatement prise en compte
        par conf/distributions (SignWith:), sinon les Release APT continuent
        d'être publiés non signés malgré l'existence d'une clé — voir le
        commentaire dans generate_gpg_key().
        """
        mock_result = MagicMock()
        mock_result.returncode = 0
        mock_result.stderr = ""
        ...
        with patch.object(_SR_MOD_B, "_ensure_gnupg_permissions"):
            with patch("subprocess.run", return_value=mock_result):
                with patch(
                    "routers.distributions_router.init_distributions",
                    return_value={"repo_format": "apt", "results": []},
                ) as mock_reinit:
                    resp = sr_client.post("/settings/gpg/generate")
        assert resp.status_code == 200
        mock_reinit.assert_called_once_with("admin")
        assert resp.json()["distributions_reinit"] == {"repo_format": "apt", "results": []}

    def test_generate_gpg_key_reinit_failure_does_not_fail_request(self, sr_client):
        """Un échec de la ré-init ne doit jamais faire échouer la génération
        de clé elle-même — la clé existe déjà à ce stade."""
        mock_result = MagicMock()
        mock_result.returncode = 0
        mock_result.stderr = ""
        mock_result.stdout = "-----BEGIN PGP"
        with patch.object(_SR_MOD_B, "_ensure_gnupg_permissions"):
            with patch("subprocess.run", return_value=mock_result):
                with patch(
                    "routers.distributions_router.init_distributions",
                    side_effect=RuntimeError("reprepro not ready"),
                ):
                    resp = sr_client.post("/settings/gpg/generate")
        assert resp.status_code == 200
        assert resp.json()["distributions_reinit"] is None
