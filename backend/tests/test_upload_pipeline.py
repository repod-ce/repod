...
class TestRepreproService:
    ...
    def test_default_is_direct_no_docker(self, mod):
        """
        Le défaut (sans passer via_docker) doit appeler reprepro directement,
        sans docker exec — la production ne monte pas /var/run/docker.sock
        dans le conteneur backend (voir docker-compose.yaml), donc
        via_docker=True ne peut pas fonctionner en déploiement par défaut.
        """
        captured = []
        def capture(cmd, **kwargs):
            captured.append(cmd)
        ...
        with patch("subprocess.run", side_effect=capture):
            mod.remove_package("mypkg", distributions=["jammy"])
