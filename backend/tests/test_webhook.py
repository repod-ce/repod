...
def _dependabot_alert_payload(
    ...
    """Payload GitHub dependabot_alert webhook (format réel — voir
    https://docs.github.com/en/webhooks/webhook-events-and-payloads#dependabot_alert)."""
    advisory: dict = {
        "ghsa_id": "GHSA-aaaa-bbbb-cccc",
        "summary": "SQL injection in QuerySet.explain()",
        "severity": severity,
    }
    if include_cve:
        advisory["cve_id"] = cve_id
...
# ═══════════════════════════════════════════════════════════════════════════════
# 3bis. Parsing GitHub Dependabot Alert (parse_dependabot_alert)
# ═══════════════════════════════════════════════════════════════════════════════

class TestParseDependabotAlert:
    """
    Le docstring de routers/webhook_router.py prétendait déjà gérer les
    Dependabot alerts ("Reçoit les webhooks GitHub (security_advisory,
    Dependabot alerts)"), mais aucune fonction de parsing n'existait pour
    l'événement dependabot_alert — il tombait systématiquement dans la
    branche par défaut {"status": "ignored"}. Ces tests couvrent le
    correctif : parse_dependabot_alert(), symétrique de
    parse_github_advisory() mais sur le format réel de ce webhook (alerte
    scopée à un paquet + manifeste précis, pas une GHSA globale).
    """

    def test_valid_alert_returns_dict(self):
        from services.webhook import parse_dependabot_alert
        result = parse_dependabot_alert(_dependabot_alert_payload())
        assert result is not None
        assert isinstance(result, dict)

    def test_cve_id_extracted(self):
        from services.webhook import parse_dependabot_alert
        result = parse_dependabot_alert(_dependabot_alert_payload(cve_id="CVE-2024-5678"))
        assert result["cve_id"] == "CVE-2024-5678"

    def test_missing_cve_returns_none(self):
        """Une alerte référençant une GHSA sans CVE assignée → None (rien à
        corréler avec les manifests internes, indexés par ID CVE)."""
        from services.webhook import parse_dependabot_alert
        payload = _dependabot_alert_payload(include_cve=False)
        assert parse_dependabot_alert(payload) is None

    def test_package_and_ecosystem_extracted(self):
        """Contrairement à security_advisory (GHSA globale), un Dependabot
        alert est toujours scopé à un paquet + écosystème précis — doit être
        exposé pour le contexte d'audit."""
        from services.webhook import parse_dependabot_alert
        result = parse_dependabot_alert(_dependabot_alert_payload(package="django", ecosystem="pip"))
        assert result["package"] == "django"
        assert result["ecosystem"] == "pip"

    def test_manifest_path_extracted(self):
        from services.webhook import parse_dependabot_alert
        result = parse_dependabot_alert(_dependabot_alert_payload(manifest_path="backend/requirements.txt"))
        assert result["manifest_path"] == "backend/requirements.txt"

    def test_alert_state_extracted(self):
        from services.webhook import parse_dependabot_alert
        result = parse_dependabot_alert(_dependabot_alert_payload(state="fixed"))
        assert result["alert_state"] == "fixed"

    def test_severity_preserved(self):
        from services.webhook import parse_dependabot_alert
        result = parse_dependabot_alert(_dependabot_alert_payload(severity="critical"))
        assert result["severity"] == "critical"

    def test_action_preserved(self):
        from services.webhook import parse_dependabot_alert
        result = parse_dependabot_alert(_dependabot_alert_payload(action="reopened"))
        assert result["action"] == "reopened"

    def test_source_field_identifies_dependabot(self):
        from services.webhook import parse_dependabot_alert
        result = parse_dependabot_alert(_dependabot_alert_payload())
        assert result["source"] == "github_dependabot_alert"

    def test_empty_payload_returns_none(self):
        from services.webhook import parse_dependabot_alert
        assert parse_dependabot_alert({}) is None
...
class TestWebhookRouterSource:
    ...
    def test_dependabot_alert_event_wired(self):
        """L'événement dependabot_alert doit être géré explicitement, pas
        seulement mentionné dans le docstring — voir TestDependabotAlertEndToEnd
        pour la preuve fonctionnelle (pas juste une inspection de source)."""
        src = self._src()
        assert "dependabot_alert" in src
        assert "parse_dependabot_alert" in src


# ═══════════════════════════════════════════════════════════════════════════════
# 6bis. Endpoint /webhooks/github — dependabot_alert de bout en bout
# ═══════════════════════════════════════════════════════════════════════════════
...
class TestDependabotAlertEndToEnd:
    """Preuve fonctionnelle réelle (pas de l'inspection de source) que
    POST /webhooks/github avec X-GitHub-Event: dependabot_alert est traité,
    signature HMAC comprise — avant le correctif ceci retournait toujours
    {"status": "ignored"}."""

    def test_valid_alert_is_processed_not_ignored(self):
        client = _webhook_client()
        payload = _dependabot_alert_payload(cve_id="CVE-2024-7777")
        ...
        assert data["cve_id"] == "CVE-2024-7777"

    def test_alert_without_cve_is_ignored_not_errored(self):
        client = _webhook_client()
        payload = _dependabot_alert_payload(include_cve=False)
        body = json.dumps(payload).encode()
    ...
    def test_security_advisory_still_processed_after_dependabot_wiring(self):
        """Non-régression : l'ajout de la branche dependabot_alert ne doit
        pas casser le chemin security_advisory existant."""
        client = _webhook_client()
        payload = _github_advisory_payload(cve_id="CVE-2024-8888")
        body = json.dumps(payload).encode()
        ...
        data = resp.json()
        assert data["status"] == "processed"
        assert data["cve_id"] == "CVE-2024-8888"
